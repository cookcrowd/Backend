...
class RecipeMapper extends BaseMapper {
  ...
  /**
  * Get latest $limit recipes
  *
  * @param int $limit (Optional) Limit the number of recipes, by default 10
  * @return array|false
  */
  ...
  */
  ...
  public function insert($recipe) {
    ...
    try {
      $sql = '
        INSERT INTO `recipes` (`title`, `preparation_time`, `image`, `created`)
        VALUES (:title, :preparation_time, :image, NOW())
      ';
      $stmt = $this->_db->prepare($sql);
      $stmt->execute(array(
